...
export async function POST(
  ...
  try {
    console.log('[whitelabel-elevenlabs-test/route] Starting test configuration for agent:', params.agentId);

    // Verify customer authentication
    const authResult = await verifyWhitelabelAuth(request);
    if (!authResult || !authResult.customerId) {
      console.log('[whitelabel-elevenlabs-test/route] Authentication failed');
    ...
    console.log('[whitelabel-elevenlabs-test/route] Authenticated customer:', customerId);

    // Get the ElevenLabs agent from database
    const agent = await prisma.elevenLabsAgent.findFirst({
      where: {
        id: params.agentId,
        // Check if agent is mapped to this customer
        customerId: customerId
      }
    });

    if (!agent) {
      console.log('[whitelabel-elevenlabs-test/route] Agent not found or not accessible:', params.agentId);
      return NextResponse.json(
    ...
    console.log('[whitelabel-elevenlabs-test/route] Found agent:', agent.name);

    // Check if agent has API key
    if (!agent.apiKey) {
      console.log('[whitelabel-elevenlabs-test/route] Agent has no API key');
      return NextResponse.json(
        { 
          error: 'No API key configured',
          message: 'This agent needs an API key to be tested. Please contact your provider to configure the API key.'
        },
        { status: 400 }
      );
    }
    ...
    try {
      console.log('[whitelabel-elevenlabs-test/route] Requesting signed URL from ElevenLabs API');
      ...
      if (!elevenLabsResponse.ok) {
        ...
        console.error('[whitelabel-elevenlabs-test/route] ElevenLabs API error:', elevenLabsResponse.status, errorText);
      ...
      console.log('[whitelabel-elevenlabs-test/route] Successfully got signed URL from ElevenLabs');
      ...
      return NextResponse.json({
        ...
        testInstructions: {
          ...
          keyInfo: 'Testing with configured API key.'
        }
      });

    } catch (elevenLabsError: any) {
      console.error('[whitelabel-elevenlabs-test/route] Error calling ElevenLabs API:', elevenLabsError);
      return NextResponse.json(
        { 
    ...
    console.error('[whitelabel-elevenlabs-test/route] Unexpected error:', error);
    return NextResponse.json(
      { 
...
export async function GET(
  ...
  try {
    console.log('[whitelabel-elevenlabs-test/route] Getting test info for agent:', params.agentId);
    ...
    console.error('[whitelabel-elevenlabs-test/route] Error getting test info:', error);
    return NextResponse.json(
      { 
        error: 'Internal server error',
