...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!payload || payload.partnerId !== userId) {
      return NextResponse.json(
    ...
    const existingCredentials = partner.passkeyCredentials || [];
    ...
    // Update partner record with transaction for atomicity
    await prisma.$transaction(async (tx) => {
      await tx.partner.update({
        ...
        data: {
          ...
          passkeySetupAt: partner.passkeyCredentials.length === 0 ? new Date() : undefined,
        },
      });
    });
    ...
    clearRateLimit(`partner-passkey-register:${clientIP}`);
