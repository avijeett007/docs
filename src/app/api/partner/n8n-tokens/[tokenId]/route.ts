...
/**
 * GET /api/partner/n8n-tokens/[tokenId]
 * Get a specific N8N token
 */
export async function GET(
  ...
  try {
    ...
    // Get token with customer info - filter by tokenType: 'n8n'
    ...
    console.error('N8N token retrieval error:', error);
    return NextResponse.json(
...
/**
 * PATCH /api/partner/n8n-tokens/[tokenId]
 * Update a specific N8N token
 */
export async function PATCH(
  ...
  try {
    ...
    // Check if token exists and belongs to partner - filter by tokenType: 'n8n'
    // Note: Legacy tokens have default 'n8n' tokenType, so no need for null check
    ...
    console.error('N8N token update error:', error);
...
/**
 * DELETE /api/partner/n8n-tokens/[tokenId]
 * Revoke/delete a specific N8N token
 */
export async function DELETE(
  ...
  try {
    ...
    console.error('N8N token deletion error:', error);
    return NextResponse.json(
      { success: false, error: 'Internal server error' },
      { status: 500 }
    );
