...
export async function POST(
  ...
  try {
    ...
    if (!userOnboarding) {
      console.log('UserOnboarding not found or not associated with this partner');
      return NextResponse.json(
        {
          success: false,
    ...
    if (!userOnboarding.customerId || !userOnboarding.customer) {
      console.log('UserOnboarding does not have an associated Customer record');
      return NextResponse.json(
        {
          success: false,
          error: 'Customer record not found. Please ensure the customer is properly set up.'
        },
        { status: 404 }
      );
    }
    ...
    await registerAgentInAnalytics({
      ...
      console.error(`[retell-map-customer/route] Error updating agent ${params.agentId} in analytics:`, error);
