...
export async function PATCH(
  ...
  try {
    ...
    // Call the analytics PATCH endpoint to update config
    ...
    const analyticsResponse = await fetch(
      `${analyticsApiUrl}/api/agents/${agentId}?provider=retell`,
      {
        method: 'PATCH',
        headers: {
...
export async function GET(
  ...
  try {
    ...
    if (!agent.analyticsAgentId) {
      return NextResponse.json({ min_call_duration: 1 }); // default
    }
    ...
    if (!analyticsResponse.ok) {
      return NextResponse.json({ min_call_duration: 1 }); // default on error
    }
