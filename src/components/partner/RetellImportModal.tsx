...
import { FiX, FiSearch, FiDownload, FiCheck, FiEye, FiEyeOff, FiSettings, FiLayers } from 'react-icons/fi';
...
interface RetellImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImportComplete: () => void;
}

export default function RetellImportModal({ isOpen, onClose, onImportComplete }: RetellImportModalProps) {
  ...
  const handleFetchAgents = async () => {
    if (!apiKey.trim()) {
      toast.error('Please provide a Retell API key');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.availableForImport === 0) {
          toast('All Retell agents have already been imported', {
            icon: 'ℹ️',
            style: {
              background: '#3B82F6',
              color: 'white',
          ...
          toast.success(`Found ${data.availableForImport} Retell agents available for import`);
        }
      } else {
        const error = await response.json();
        toast.error(error.error || 'Failed to fetch Retell agents');
      }
    } catch (error) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Successfully imported ${result.summary.successful} Retell agents!`);
  ...
  // Deduplicate agents: group by id, keep only the latest version by default
  const deduplicatedAgents = useMemo(() => {
    if (showAllVersions) return agents;

    const latestById = new Map<string, ImportableAgent>();
  ...
  const hasMultipleVersions = useMemo(() => {
    ...
    for (const agent of agents) {
      versionCounts.set(agent.id, (versionCounts.get(agent.id) || 0) + 1);
    }
    return Array.from(versionCounts.values()).some(count => count > 1);
  }, [agents]);

  const filteredAgents = deduplicatedAgents.filter(agent => {
  ...
  const availableAgents = deduplicatedAgents.filter(agent => !agent.alreadyImported);
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-400 mb-6">
                    Provide your Retell API key to fetch available agents for import.
                  </p>
                </div>
            ...
            {step === 'agents' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-medium text-white mb-2">Select Retell Agents to Import</h3>
                    <p className="text-gray-400">
                      Found {deduplicatedAgents.length} agent{deduplicatedAgents.length !== 1 ? 's' : ''} ({availableAgents.length} available for import)
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
                  ...
                  {filteredAgents.map((agent) => (
                    <div
                      key={`${agent.id}-v${agent.version ?? 0}`}
                      className={clsx(
                      ...
                      onClick={() => !agent.alreadyImported && handleAgentToggle(agent.id)}
                    >
                            ...
                            <div className="space-y-1">
                              {agent.details?.voiceId && (
                                <div className="text-xs text-gray-500">
                                  Voice: {agent.details.voiceId}
                                </div>
                              )}
                              {agent.details?.language && (
                                <div className="text-xs text-gray-500">
                                  Language: {agent.details.language}
                                </div>
                              )}
                              {agent.lastModified && (
                                <div className="text-xs text-gray-500">
            ...
            {step === 'importing' && (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-white mb-2">Importing Retell Agents...</h3>
                <p className="text-gray-400">
                  Please wait while we import {selectedAgents.size} agent{selectedAgents.size !== 1 ? 's' : ''} from Retell AI.
                </p>
                <div className="mt-4 text-sm text-gray-500">
                  This includes registering agents with analytics and configuring webhooks.
                </div>
              </div>
            )}
          </div>
