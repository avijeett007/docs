...
interface VapiImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImportComplete: () => void;
}

export default function VapiImportModal({ isOpen, onClose, onImportComplete }: VapiImportModalProps) {
  ...
  const handleFetchAgents = async () => {
    if (!apiKey.trim()) {
      toast.error('Please provide a VAPI API key');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.availableForImport === 0) {
          toast('All VAPI agents have already been imported', {
            icon: 'ℹ️',
            style: {
              background: '#3B82F6',
              color: 'white',
          ...
          toast.success(`Found ${data.availableForImport} VAPI agents available for import`);
        }
      } else {
        const error = await response.json();
        toast.error(error.error || 'Failed to fetch VAPI agents');
      }
    } catch (error) {
      console.error('Error fetching VAPI agents:', error);
      toast.error('Failed to fetch VAPI agents');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Successfully imported ${result.summary.successful} VAPI agents!`);
        ...
        const error = await response.json();
        console.log('Import error response:', { status: response.status, error });
        ...
        if (response.status === 403 && error.validation &&
          ...
          console.log('Showing upgrade modal for limit error');
          // Show upgrade modal instead of just error toast
          setUpgradeModalData({
            limitType: 'VAPI agents',
  ...
  const getVoiceString = (agent: ImportableAgent): string => {
    ...
    if (typeof d.voice === 'string') return d.voice;
    return '';
  };
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-400 mb-6">
                    Provide your VAPI API key to fetch available agents for import.
                  </p>
                </div>
            ...
            {step === 'agents' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-medium text-white mb-2">Select VAPI Agents to Import</h3>
            ...
            {step === 'importing' && (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-white mb-2">Importing VAPI Agents...</h3>
                <p className="text-gray-400">
                  Please wait while we import {selectedAgents.size} agent{selectedAgents.size !== 1 ? 's' : ''} from VAPI.
                </p>
