...
interface UltravoxImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImportComplete: () => void;
}

export default function UltravoxImportModal({ isOpen, onClose, onImportComplete }: UltravoxImportModalProps) {
  ...
  const handleFetchAgents = async () => {
    if (!apiKey.trim()) {
      toast.error('Please provide an Ultravox API key');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.availableForImport === 0) {
          toast('All Ultravox agents have already been imported', {
            icon: 'ℹ️',
            style: {
              background: '#3B82F6',
              color: 'white',
          ...
          toast.success(`Found ${data.availableForImport} Ultravox agents available for import`);
        }
      } else {
        const error = await response.json();
        toast.error(error.error || 'Failed to fetch Ultravox agents');
      }
    } catch (error) {
      console.error('Error fetching Ultravox agents:', error);
      toast.error('Failed to fetch Ultravox agents');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Successfully imported ${result.summary.successful} Ultravox agents!`);
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-medium text-white mb-2">
                    Enter Ultravox API Key
                  </h3>
                  <p className="text-gray-400 mb-6">
                    Provide your Ultravox API key to fetch available agents for import.
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      ...
                      <span>Fetch Ultravox Agents</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            {step === 'agents' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-medium text-white mb-2">Select Ultravox Agents to Import</h3>
                            ...
                            <div className="space-y-1">
                              {agent.details?.voice && (
                                <div className="text-xs text-gray-500">
                                  Voice: {agent.details.voice.provider || 'Unknown'}
                                </div>
                              )}
                              {agent.details?.model && (
                                <div className="text-xs text-gray-500">
                                  Model: {agent.details.model.provider || 'Unknown'}
                                </div>
                              )}
                              {agent.status && (
                                <div className="text-xs text-gray-500">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-300 leading-relaxed">
                          During import, we'll add a tracking identifier to each agent's system prompt to enable webhook analytics.
                          This identifier is invisible to users and doesn't affect the AI's responses. It allows us to track which
                          agent generated each call for accurate analytics and billing.
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            {step === 'importing' && (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-white mb-2">Importing Ultravox Agents...</h3>
                <p className="text-gray-400">
                  Please wait while we import {selectedAgents.size} agent{selectedAgents.size !== 1 ? 's' : ''} from Ultravox.
                </p>
