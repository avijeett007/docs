...
import { FiClock, FiPhoneCall, FiDollarSign, FiCheckCircle, FiXCircle, FiFilter } from 'react-icons/fi';
import {
...
interface Agent {
  ...
  assistantId: string;
  customerId?: string;
  type?: 'vapi' | 'retell';  // Keep for internal use only
}
...
const DetailedUsage: React.FC = () => {
  ...
  const [allAgents, setAllAgents] = useState<AgentUsage[]>([]);
  ...
  useEffect(() => {
    ...
    const loadAllAgents = async () => {
      ...
      try {
        ...
        // Combine all agents
        const combined = [...vapiAgents, ...retellAgents];
  ...
  const fetchAnalytics = async (agent: AgentUsage, period: string) => {
    ...
    try {
      const endpoint = agent.type === 'vapi'
        ? `/api/customer/vapi-agents/${agent.id}/analytics?period=${period}`
        : `/api/customer/retell-agents/${agent.id}/analytics?period=${period}`;
  ...
  const DetailedView = ({ agent }: { agent: AgentUsage }) => {
    ...
    const processTimeSeriesData = useCallback(() => {
      try {
        ...
        // Use all available data for time series visualization
        let filteredData = data;

        if (agent.type === 'vapi') {
          // Filter to include only completed calls
        ...
        // Log the total count from time series data
        ...
        // If the time series data shows more calls than the analytics, log a warning
        if (totalCount > agent.analytics.callCount) {
          console.warn(`Time series data shows ${totalCount} calls, but analytics only reports ${agent.analytics.callCount}. This may indicate that the analytics API is not fetching all calls.`);
        }
    ...
    const fetchTimeSeriesData = async () => {
      ...
      try {
        ...
        // The API now handles pagination internally and returns all calls
        const period = 'month'; // Default to month view
        const response = await fetch(`/api/customer/conversations?agentId=${agent.id}&period=${period}&agentType=${agent.type}`);
        ...
        console.log(`Fetched ${responseData.data.length} calls for time series data`);
        if (responseData.pagination) {
          console.log(`Pagination info: Total fetched: ${responseData.pagination.totalFetched}, Pages processed: ${responseData.pagination.pagesProcessed}`);
        }
