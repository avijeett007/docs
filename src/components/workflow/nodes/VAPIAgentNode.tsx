...
import { FiPhone, FiSettings, FiMic, FiUser, FiEdit3 } from 'react-icons/fi';
import { WorkflowNode, VAPIAgentData } from '@/types/workflow';

interface VAPIAgentNodeProps {
  node: WorkflowNode;
...
export const VAPIAgentNode: React.FC<VAPIAgentNodeProps> = ({
  node,
  onUpdate,
  onConfigClick,
  onDrag,
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    // Start drag immediately but allow for quick clicks
    startDrag();
    onSelect?.();
  ...
  const loadAvailableAgents = async () => {
    try {
      ...
      const mockAgents: VAPIAgentData[] = [
        {
          id: 'vapi_agent_1',
  ...
  const handleAgentSelect = (agentId: string) => {
    ...
    if (selectedAgent) {
      onUpdate(node.id, {
        ...
        name: `VAPI: ${selectedAgent.name}`
      });
    }
  };

  const selectedAgent = node.data.agentData as VAPIAgentData | undefined;

  return (
    <div
      ref={nodeRef}
      className={`
        relative bg-gradient-to-br from-blue-600 to-blue-700 rounded-lg shadow-lg border-2 transition-all duration-200
        ${isSelected ? 'border-blue-300 shadow-blue-300/50' : 'border-blue-500/50'}
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-blue-500/30">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-white font-medium text-sm">VAPI Agent</h3>
            <p className="text-blue-200 text-xs">Voice AI Platform</p>
          </div>
        </div>
        <button
          onClick={() => onConfigClick(node.id)}
          className="p-1.5 text-blue-200 hover:text-white hover:bg-blue-500/30 rounded transition-colors"
        >
        ...
        {!node.data.agentId ? (
          <div className="space-y-3">
            ...
            {loading ? (
              <div className="text-center py-4">
                <div className="animate-spin w-5 h-5 border-2 border-blue-300 border-t-transparent rounded-full mx-auto"></div>
                <p className="text-blue-200 text-xs mt-2">Loading agents...</p>
              </div>
            ) : (
              <select
                value={node.data.agentId || ''}
                onChange={(e) => handleAgentSelect(e.target.value)}
                className="w-full px-3 py-2 bg-blue-800/50 border border-blue-500/50 rounded-lg text-white text-sm focus:outline-none focus:border-blue-400"
              >
            ...
            {availableAgents.length === 0 && !loading && (
              <p className="text-blue-300 text-xs text-center py-2">
                No available VAPI agents found
              </p>
            )}
          </div>
        ) : (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => onUpdate(node.id, { agentId: undefined, agentData: undefined, name: 'VAPI Agent' })}
            ...
            {selectedAgent && (
              <div className="space-y-2 text-xs">
                <div className="flex items-center gap-2">
                  <FiPhone className="w-3 h-3 text-blue-300" />
                  <span className="text-blue-200">
                    Voice: {selectedAgent.voice?.provider || 'Default'}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-green-400"></div>
                  <span className="text-blue-200">
                    Model: {selectedAgent.model?.model || 'Default'}
                  </span>
                </div>
                {selectedAgent.recordingEnabled && (
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-red-400"></div>
                    <span className="text-blue-200">Recording Enabled</span>
                  </div>
                )}
              </div>
            )}
      ...
      >
        <div className="w-8 h-8 bg-blue-500 rounded-full border-4 border-white shadow-xl hover:bg-blue-400 hover:scale-125 transition-all duration-200 flex items-center justify-center">
