...
import { FiPhone, FiKey, FiMic, FiUser, FiPlus, FiRefreshCw } from 'react-icons/fi';
import PositionedModal from './PositionedModal';
import { RetellAgentData } from '@/types/workflow';

interface EnhancedRetellAgentConfigModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (config: RetellAgentData) => void;
  initialConfig?: Partial<RetellAgentData>;
  nodePosition?: { x: number; y: number };
  customerId?: string;
}

export default function EnhancedRetellAgentConfigModal({
  isOpen,
  onClose,
  onSave,
  initialConfig = {},
  nodePosition,
  customerId,
}: EnhancedRetellAgentConfigModalProps) {
  const [step, setStep] = useState<'select' | 'import' | 'configure'>('select');
  const [selectedAgent, setSelectedAgent] = useState<RetellAgentData | null>(null);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialConfig.id) {
        ...
        setSelectedAgent(initialConfig as RetellAgentData);
        setStep('configure');
      }
    }
  ...
  const loadAvailableAgents = async () => {
    console.log('loadAvailableAgents called with customerId:', customerId);
    setLoading(true);
    try {
      ...
      if (response.ok) {
        ...
        console.log('First agent structure:', data[0] || data.data?.[0]);
        ...
        console.log('All agents for partner:', allAgents);
  ...
  const handleImportAgent = async () => {
    ...
    try {
      ...
      console.error('Error importing Retell agent:', error);
    } finally {
      setLoading(false);
    }
  };

  const handleSelectAgent = (agent: RetellAgentData) => {
  ...
  const renderSelectStep = () => (
    <div className="p-6 space-y-6">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-white mb-2">Select Retell Agent</h3>
        ...
        <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
                {availableAgents.map((agent) => (
                  <div
                    ...
                    className="p-4 bg-gray-700 border border-gray-600 rounded-lg cursor-pointer hover:border-green-500 transition-colors"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <h5 className="font-medium text-white truncate">{agent.name}</h5>
                        <p className="text-sm text-gray-400 truncate">Voice: {agent.voiceId || 'Default'}</p>
                        <p className="text-sm text-gray-400 truncate">Language: {agent.language || 'en-US'}</p>
                      ...
                      <FiUser className="w-5 h-5 text-green-400 flex-shrink-0 ml-2" />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="border-t border-gray-600 pt-4">
            <button
              ...
              className="w-full p-4 border-2 border-dashed border-gray-600 rounded-lg hover:border-green-500 transition-colors group"
            >
              <div className="flex items-center justify-center gap-2 text-gray-400 group-hover:text-green-400">
                <FiPlus className="w-5 h-5" />
                <span>Import New Retell Agent</span>
              </div>
            </button>
          </div>
          ...
          {availableAgents.length === 0 && (
            <div className="text-center py-8 text-gray-400">
              ...
              <p>No Retell agents available. Import one to get started.</p>
            </div>
          )}
        </div>
      )}
  ...
  const renderImportStep = () => (
    <div className="p-6 space-y-6">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-white mb-2">Import Retell Agent</h3>
        <p className="text-gray-400">Enter your Retell API key and agent ID</p>
      </div>
  ...
  const renderConfigureStep = () => (
    <div className="p-6 space-y-6">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-white mb-2">Configure Retell Agent</h3>
      ...
      {selectedAgent && (
        <div className="space-y-4">
            ...
            <div className="p-4 bg-gray-800/50 border border-gray-600 rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white">{selectedAgent.voiceId || 'Default'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Language:</span>
                  <span className="text-white">{selectedAgent.language || 'en-US'}</span>
                </div>
              </div>
            </div>
          ...
          {selectedAgent.voiceModel && (
            <div className="p-4 bg-gray-800/50 border border-gray-600 rounded-lg">
              <h4 className="text-sm font-medium text-gray-300 mb-2">Voice Configuration</h4>
              <p className="text-sm text-white">Model: {selectedAgent.voiceModel}</p>
            </div>
          )}
        </div>
      )}
