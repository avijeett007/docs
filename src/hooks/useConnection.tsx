...
export const ConnectionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const disconnect = useCallback(async () => {
    console.log('🔴 useConnection: Initiating disconnect...');
    
    // Only proceed with disconnect if we're actually connected
    if (!connectionDetails.shouldConnect) {
      console.log('ℹ️ useConnection: Already disconnected, skipping...');
      return;
    }
    
    // First update the connection state
    setConnectionDetails({
      wsUrl: "",
      token: "",
      shouldConnect: false,
      mode: "manual"  // Reset to default mode
    });
    
    console.log('🏁 useConnection: Connection state cleared');
