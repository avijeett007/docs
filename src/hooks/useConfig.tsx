...
export const ConfigProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const getSettingsFromUrl = useCallback(() => {
    ...
    // Only read hash parameters on playground/demo pages
    const pathname = window.location.pathname;
    const isPlaygroundPage = pathname.includes('/playground') || 
                             pathname.includes('/demo') ||
                             pathname.includes('/whitelabel/embed');
  ...
  const setUrlSettings = useCallback(
    (us: UserSettings) => {
      // Only set hash parameters on playground/demo pages, not on marketing pages
      if (typeof window !== 'undefined') {
        const pathname = window.location.pathname;
        const isPlaygroundPage = pathname.includes('/playground') || 
                                 pathname.includes('/demo') ||
        ...
        if (!isPlaygroundPage) {
          // Don't add hash parameters to non-playground pages
          return;
        }
      }
