...
> ![Knotie AI Pro Partner Portal — Phone Numbers list with assigned customers (captured 2026-06-09; the same screenshot is reused in the warm-lead-GHL and Retell import use cases — this caption frames it as the agency-customer-assigned view of the same list)](/images/screenshots/phone-numbers-2026-06-09.png)
...
> ℹ️ This step happens inside GoHighLevel. Add a **Webhook** action with Method `POST`, URL `https://connecthub.knotie-ai.pro/api/outbound-call`, an `Authorization: Bearer owk_...` header, and the `from_number` / `to_number` / `metadata` body fields. See the [warm-lead-outbound-calls-ghl](/use-cases/warm-lead-outbound-calls-ghl) playbook for a full GHL webhook walkthrough.
