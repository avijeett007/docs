...
<Warning>
**The install path described in earlier versions of this doc is stale.** The standalone `VPS → App Catalog` page is still linked from the docs nav and from **Related pages** below, but as of this writing it is not reliably reachable in the live Partner Portal — the canonical path has moved. Treat that page as a fallback rather than the primary entry point.

To install Hermes today, try one of these paths in order:
