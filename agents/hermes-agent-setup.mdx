...
{/*
The VPS App Catalog is reachable from any active VPS instance's detail page (the App Catalog button next to Reinstall). With zero VPS instances, the App Catalog cannot be opened, and the standalone /partner/vps/app-catalog URL is currently 404. The Hermes Agent card has a BETA badge and a Deploy button in the catalog grid; capture is blocked until at least one VPS instance is provisioned.
*/}
...
{/*
The Hermes Agent pre-deploy form modal is opened by clicking Deploy on the Hermes Agent card in the VPS App Catalog. While the App Catalog is unreachable (see the note above), the pre-deploy form is also unreachable.
*/}
