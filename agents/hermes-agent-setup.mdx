...
<Info>
The AI Chat page with the Hermes Agent ⚡ selector only appears on the whitelabel portal of a customer that has an active VPS instance with Hermes Agent deployed via the VPS App Catalog. The KnoLabs AI Agency account has zero VPS instances, and the App Catalog page (`/partner/vps/app-catalog`) returns 404 on this account (see [Journey #11](/journeys/11-vps) for the App Catalog 404). To capture, provision at least one VPS instance and complete the Hermes Agent catalog deploy.
</Info>
...
<Info>
The VPS App Catalog is reachable from any active VPS instance's detail page (the **App Catalog** button next to **Reinstall**). With zero VPS instances in the KnoLabs account, the App Catalog cannot be opened. Additionally, the standalone `/partner/vps/app-catalog` URL is currently 404 in this account (see [Journey #11](/journeys/11-vps)). The Hermes Agent card has a `BETA` badge and a **Deploy** button in the catalog grid; capture is blocked until at least one VPS instance is provisioned.
</Info>
...
<Info>
The Hermes Agent pre-deploy form modal is opened by clicking **Deploy** on the Hermes Agent card in the VPS App Catalog. Since the App Catalog is unreachable on this account (see the note above), the pre-deploy form is also unreachable.
</Info>
...
<Info>
The whitelabel portal AI Chat page with the Hermes Agent ⚡ option and a sample exchange requires an end-customer who has an active VPS instance with the Hermes Agent catalog deploy complete. The KnoLabs account has no VPS instances and no customer with a deployed Hermes Agent, so the AI Chat page is unreachable in a state that shows the ⚡ Hermes Agent selector. To capture, complete the full deploy flow on a test customer's whitelabel portal.
</Info>
...
<Info>
The Telegram bot integration requires creating a bot via [@BotFather](https://t.me/BotFather) and supplying the bot token + chat ID during the Hermes Agent pre-deploy form. With no VPS instance and no deployed Hermes Agent, the Telegram messaging gateway is not running and no conversation can be captured. To capture, complete the deploy with a real Telegram bot token and exchange at least one message with the bot.
</Info>
