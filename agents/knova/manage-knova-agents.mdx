...
If you use Knotie's MCP server (the integration gateway) with AI coding tools like Cursor or Claude, note these updated tool schemas:
...
- **Agent config not updating after edit** — The platform invalidates the integration gateway agent cache after every save. If the agent's phone number still routes to old config, wait 30 seconds and call the number again. If it persists, contact support.
