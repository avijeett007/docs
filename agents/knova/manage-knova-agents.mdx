...
> 📷 **Screenshot needed:** _Add a screenshot of the "Create Agent" modal with the Knova type selected and the Basic tab visible._

1. In the Partner Portal, open **AI Agents**.
2. Click **Create Agent** (top right).
3. In the **Create Agent** modal, choose one of two creation modes:
   - **AI Builder** – Chat with the AI to generate a system prompt and agent config automatically. Recommended for new agents.
   - **Manual** – Fill in the form fields directly.
4. If you chose Manual, complete the **Basic** tab:
   - **Customer** – Select the customer this agent belongs to.
   - **Agent Name** – A short internal name (max 100 characters).
   - **System Prompt** – The instructions your agent follows on every call (max 15,000 characters).
   - **Greeting Message** – The first thing the agent says when a call connects.
   - **Voice** – Pick a voice from the list (see [Voice selection](#voice-selection) below).
5. Configure optional tabs (**Tools**, **Knowledge Base**, **Advanced**) as needed.
6. Click **Save** to create the agent.

> 🎬 **Video walkthrough:** _Record a short clip showing the full agent creation flow — selecting a customer, entering a system prompt, choosing a voice, and saving._
...
> 📷 **Screenshot needed:** _Add a screenshot of the voice picker in the Create/Edit Agent modal, showing the blue and purple dot indicators next to voice names._
...
> 📷 **Screenshot needed:** _Add a screenshot of the "Resume a previous session" cards at the top of the AI Builder Chat modal._

## Assign a phone number

After creating a Knova agent you can link a phone number to handle inbound calls:

1. After saving the agent, click **Assign Phone Number** in the confirmation screen, or open the agent later and click **Edit → Phone Assignment**.
2. Select a provisioned number from your pool.
3. Click **Assign**.
...
- [MCP Tokens](/partner-portal/settings/mcp-tokens) – Set up MCP access for AI coding tools
