...
The value must match the `CONNECT_HUB_API_KEY` environment variable in Connect Hub. This endpoint is used by the partner portal's test-call feature and is not intended for direct use by partners.
...
GoHighLevel workflows send a flat `customData` envelope. Connect Hub detects this automatically — no configuration needed.
