...
{/*
The mcpServers entry in advancedConfig only appears for Knova agents that have a customer assigned and the integration gateway configured. A website-channel agent with no customer assignment does not include an mcpServers block in its Advanced Config panel. To capture, assign a customer to a Knova agent and open the Advanced tab in the edit modal — the JSON config will show the auto-injected knotie_partner_tools MCP server with streamable_http transport.
*/}
...
- **Integration gateway** must be configured (`CONNECT_HUB_URL` environment variable). Knova MCP wiring is silently skipped when the integration gateway is unavailable — agent creation never fails because of this.
