...
You can test an ElevenLabs agent from the Partner Portal before assigning it to a customer.

1. Find the agent in the ElevenLabs Agents list.
2. Open the agent's action menu (⋮) and select **Test Agent**.
3. Select **Start Test Call**.
4. Allow microphone access when the browser asks.
5. Speak naturally, then use the controls shown in the dialog to mute or end the call.

![ElevenLabs agent test dialog before starting a call](/images/screenshots/elevenlabs-test-agent-2026-09-14.png)

If the call does not connect, confirm that the agent is active and review **API Key** from its action menu. If the microphone prompt does not appear, allow microphone access in the browser's site settings and reload the page.
