...
2. An ElevenLabs API key added under **Partner Portal → Settings → Integration Settings**.
...
> 📷 **Screenshot needed:** _Add a screenshot of the ElevenLabs Agents page showing the agent list and the provider card._
...
> 📷 **Screenshot needed:** _Add a screenshot of the Knowledge Base tab showing the list of knowledge bases with toggles._
...
| **System Prompt** | The LLM instruction prompt for the agent |
...
| **Max Tokens** | Token ceiling per response (`-1` = unlimited) |
...
<Note>
Function calls require a customer to be selected first (the customer's Connect Hub must be set up).
</Note>

> 🎬 **Video walkthrough:** _Record a short clip walking through creating an ElevenLabs agent end-to-end: import → assign customer → configure settings → add a function call → save._

## Manage your API key

The API key used to communicate with ElevenLabs is stored encrypted in the portal. To update or re-verify it:

1. Click the **API Key** button on the ElevenLabs agents page.
...
> 📷 **Screenshot needed:** _Add a screenshot of the API Key management modal showing the status indicator and last-verified timestamp._
...
| Symptom | Fix |
|---|---|
| Import returns no agents | Confirm your ElevenLabs API key is valid under Settings → Integration Settings |
...
![ElevenLabs Agents page showing active agent list](/images/screenshots/elevenlabs-agents-2026-06-09.png)
