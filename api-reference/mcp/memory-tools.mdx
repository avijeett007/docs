...
ℹ️ _Screenshot to add: a Claude Code session after running `tools/list`, showing `internal_memory_remember`, `internal_memory_recall`, and `internal_memory_ask` in the output._
...
ℹ️ _Screenshot to add: the Partner Portal API Keys page showing an API key with `memory:read` and `memory:write` scopes selected._
