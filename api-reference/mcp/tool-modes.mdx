...
> 📷 **Screenshot needed:** _Add a screenshot of the "Generate Token" modal in **AI Agents → MCP Tokens** if it exposes tool mode or allowed-apps options in the UI._
...
> 🎬 **Video walkthrough:** _Record a short clip showing a voice agent using `internal_search_tools` to find the `SOCIAL_POST_SCHEDULE` tool and then calling it via `internal_execute_tool`._
...
- [MCP tokens](/partner-portal/settings/mcp-tokens) — Generate and manage partner MCP tokens
