...
> ℹ️ The **Docs & Media** section renders in the customer portal at `/whitelabel/<media-route>` once the partner enables the menu item under **Customer → Features & Add-ons → Menu Visibility**. The current whitelabel deployment (`dashboard.viddescriptor.com`) returns 404 on the likely routes (`/media`, `/media-library`, `/files`, `/assets`), so the section is not yet wired up at the customer level. Capture from a build where the route is live.
...
> ℹ️ **Allocated Storage (MB)** is shipped in [knotie-ai-pro#967](https://github.com/avijeett007/knotie-ai-pro/pull/967) as part of tiered storage. The field is not present in the current Customer modal's Features & Add-ons tab for Chess Classics — only Customer Portal / Add-on Features / Premium Add-ons / AI Credit Management / Low Credit Notifications / Menu Visibility / Pricing / Team Management / Sonti CRM are visible. The partner account is on a build prior to tiered storage, so storage falls back to the partner's pool default. Capture from a build that includes the field.
