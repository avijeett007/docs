...
> ℹ️ Capturing an MCP-driven post scheduling interaction requires an active voice or chat session where the agent invokes `SOCIAL_POST_SCHEDULE`. A common pattern: a customer asks the agent "post our new product photo on Friday at 9 AM" → agent picks the asset from the media library, fills the schedule, and confirms in the chat. Transcript excerpts land in the Conversations tab — capture from there.
