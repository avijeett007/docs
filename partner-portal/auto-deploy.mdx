...
**Auto-Deploy** automatically activates AI agents for customers who clear the payment gate and click **Deploy** on Step 9 (Summary & Deploy) of the SaaS onboarding process. This eliminates manual deployment work and provides instant activation for self-service customers.

<Note>
The live onboarding flow is **9 steps** total — Step 9 is **Summary & Deploy**, which combines pricing/payment verification with the deploy button. Earlier docs that reference "Step 9 (Pricing & Payment) + Step 10 (Deploy Agent)" describe the older 10-step flow.
</Note>
...
1. **Customer completes Step 9 (Summary & Deploy — pricing/payment verified)** – Stripe Checkout clears (or is skipped for free / pay-as-you-go portals)
2. **Deploy button enabled** – `/api/whitelabel/check-onboarding-payment` confirms the customer credential is `active`; otherwise the user stays on Step 9 with an error
...
1. The page first calls `/api/whitelabel/check-onboarding-payment`; if the credential is not `active`, the customer stays on Step 9 with an error message.
