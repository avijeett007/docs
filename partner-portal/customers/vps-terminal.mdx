...
{/* > ℹ️ The whitelabel sidebar shows **Server Terminal** as a leaf item in the customer tools section once enabled. Capture from a customer portal where VPS Terminal is active — the partner-side VPS management page is shown in the screenshot below. */}
...
{/* > ℹ️ The **connect form** asks for hostname / port / username / SSH private key in that order, with an inline Test connection button before the customer can submit. Capture from a customer portal with stored credentials to refresh this placeholder. */}
...
## Phase 2 — Integration gateway migration

In a future release, VPS Terminal connectivity will be migrated to the integration gateway infrastructure, which will replace the current custom WebSocket proxy approach. This will not change the customer-facing experience.
...
![Knotie AI Pro Partner Portal — VPS management page (the VPS Terminal feature is enabled per-customer under their Features & Add-ons, and shows up in the customer whitelabel portal as "Server Terminal")](/images/screenshots/vps-page-2026-06-09.png)
