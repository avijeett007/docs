...
**Both connection types are fully supported.** Once a customer connects an app — whether via OAuth or API key — the app's tools are available to their AI agents during live calls through the integration gateway MCP server.
...
2. The integration gateway MCP server resolves the app's available tools (up to 200 per toolkit).
