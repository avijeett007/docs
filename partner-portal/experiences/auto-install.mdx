...
This usually means you've run out of server capacity. Verify you have room for another server, then re-trigger the order.

**AI credits not refilling**

Make sure your Stripe Connect account is fully connected under **Settings → Stripe Connect** so renewal payments come through. If credits still don't top up after a successful renewal, contact support.

**Agent not responding (Hermes)**

Re-run the install from **VPS → My Instances → Reinstall** — it is safe to re-run and will not wipe customer data.
